...
class iPhoneDiskApplication : Form
{
	...
	private void PopulateDeviceList()
	{
		...
		//UsbRegDeviceList RDL = UsbDevice.AllDevices;
	...
	private void USBNotifier_OnDeviceNotify(object sender, DeviceNotifyEventArgs e)
	{
		...
		// I don't know how to distinguish, at this point, whether the new device is
		// an iPhone/iTouch with libusb. PopulateDeviceList may be called sometimes when
		// an old iPod, etc., is inserted or removed. Not a big deal, but undesirable.
		if (!bDriveMounted)
		{
			PopulateDeviceList();
			TryAutomount();
