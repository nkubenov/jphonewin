...
class iPhoneWrapper : DokanOperations
{
	...
	public bool Usable
	{
		get
		{
			return !(iPhoneUSBDevice == null || iPhoneEndpointReader == null || iPhoneEndpointWriter == null);
		}
	}
	...
	// Note: actual deletion occurs in Cleanup after file is closed. This is by Dokan's design.
	public int DeleteDirectory(string sPath, DokanFileInfo DFI)
	{
		...
		if (sFiles.Length > 3) return -145; // ".", ".." and a null string. -145 is ERROR_DIR_NOT_EMPTY
		return 0;
	}
	...
	{
		// TODO: on jailbroken devices, this does not return the correct information.
		try
		{
...
abstract class USBMUXConnection
{
	...
	// Sends a packet to the iPhone made up of a packet header and byData.
	protected int Send(byte[] byData)
	{
		...
		if (byData != null)
		{
			nPacketLength += byData.Length;
			byPacket = new byte[nPacketLength];
			Array.Copy(byData, 0, byPacket, 28, byData.Length);
		}
		else
		...
		if (byData != null) SentCount += (uint)byData.Length;
	...
	protected byte[] Receive()
	{
		...
		if (BufferedPacketsWaiting.Count > 0)
		{
			...
			byData = BufferedPacketsWaiting.Dequeue();
			ReceivedCount += (uint)byData.Length;
			return byData;
		}
		...
		byData = new byte[byPacket.Length - 28];
		Array.Copy(byPacket, 28, byData, 0, byPacket.Length - 28);
	...
	public void BufferPacket(byte[] byPacket)
	{
		...
		byte[] byData = new byte[byPacket.Length - 28];
		Array.Copy(byPacket, 28, byData, 0, byPacket.Length - 28);
		BufferedPacketsWaiting.Enqueue(byData);
	}
}
...
class AFCConnection : USBMUXConnection
{
	...
	private int AFCSend(byte[] byData, AFCPacketType Type)
	{
		if (iPhone == null || !iPhone.Usable) throw new Exception("No device ready in AFCSend.");
		if (byData == null) throw new Exception("No data passed to AFCSend.");

		long nPacketLength = byData.LongLength + 40;
		...
		Array.Copy(byData, 0, bAugmentedData, 40, byData.Length);
	...
	private AFCPacketData AFCReceive()
	{
		...
		ReceivedPacket.byData = byReceivedData;
	...
	public string GetFileInfo(string sPath)
	{
		...
		if (Packet.Type == AFCPacketType.Data)
			return System.Text.Encoding.UTF8.GetString(Packet.byData);

		// Otherwise, something went wrong.
		if (Packet.Type == AFCPacketType.Status && Packet.byData != null && Packet.byData.Length == 8)
		{
			long nErrorCode = BitConverter.ToInt64(Packet.byData, 0);
			switch (nErrorCode)
			{
	...
	public void MakeDirectory(string sPath)
	{
		...
		if (Packet.Type != AFCPacketType.Status || Packet.byData.Length != 8
				|| BitConverter.ToInt64(Packet.byData, 0) != (long)AFCErrorCode.Success)
			throw new Exception();
	}
	...
	public string[] ListDirectory(string sPath)
	{
		...
		if (Packet.Type == AFCPacketType.Data)
			return System.Text.Encoding.UTF8.GetString(Packet.byData).Split(new char[] { '\x0' });
	...
	public long OpenFile(string sPath, AFCFileMode FM)
	{
		...
		if (Packet.Type == AFCPacketType.FileRefOpenResult && Packet.byData.Length == 8)
			return BitConverter.ToInt64(Packet.byData, 0);
	...
	public byte[] ReadFile(long nFileHandle, long nBytesToRead)
	{
		...
		while (nReceivedBytes < nBytesToRead)
		{
			...
			if (Packet.Type == AFCPacketType.Data && Packet.byData != null)
			{
				Array.Copy(Packet.byData, 0, byFileData, nReceivedBytes, Packet.byData.LongLength);
				nReceivedBytes += Packet.byData.LongLength;

				if (Packet.byData.Length == 0) break;
			}
			else if (Packet.Type == AFCPacketType.Status && Packet.byData != null && Packet.byData.Length == 8)
			{
				long nErrorCode = BitConverter.ToInt64(Packet.byData, 0);
				switch (nErrorCode)
				{
	...
	// Writes byData to a file.
	// Throws Exception.
	public uint WriteFile(long nFileHandle, byte[] byData)
	{
		...
		AFCMutex.WaitOne();
		while (nWrittenBytes < byData.LongLength)
		{
			long nPacketDataLength = ((byData.LongLength - nWrittenBytes) > MaxWriteSize ? MaxWriteSize : (byData.LongLength - nWrittenBytes));
			byte[] byPacket = new byte[8 + nPacketDataLength];
			Array.Copy(BitConverter.GetBytes(nFileHandle), byPacket, 8);
			Array.Copy(byData, nWrittenBytes, byPacket, 8, nPacketDataLength);
		...
		if (nWrittenBytes == byData.LongLength)
			return (uint)byData.Length;

		// Otherwise, something went wrong.
		throw new Exception("Wrote " + nWrittenBytes + " bytes. Expected to write " + byData.LongLength + ".");
	}
...
abstract class PListConnection : USBMUXConnection
{
	...
	protected int PListSend(byte[] byData)
	{
		if (iPhone == null || !iPhone.Usable) throw new Exception("No device ready in PListSend.");
		if (byData == null) throw new Exception("No data passed to PListSend.");

		byte[] bAugmentedData = new byte[4 + byData.Length];
		Array.Copy(BitConverter.GetBytes(System.Net.IPAddress.HostToNetworkOrder((int)byData.Length)), bAugmentedData, 4);
		Array.Copy(byData, 0, bAugmentedData, 4, byData.Length);
...
class LockdownConnection : PListConnection
{
	...
	// Only to be called by SSLHelperStream. byte[] byData will be SSL-encrypted
	// data that needs to be passed along to the device.
	private int LockdownSendSSLCallback(byte[] byData)
	{
		if (iPhone == null || !iPhone.Usable) throw new Exception("No device ready in LockdownSendSSLCallback.");
		if (byData == null) throw new Exception("No data passed to LockdownSendSSLCallback.");
		return Send(byData);
	}
...
class SSLHelperStream : MemoryStream
{
	...
	public delegate int WriteDelegate(byte[] byData);
